...
const (
	// LabelProperty is property name as label
	LabelProperty = "label"

	// LabelEqualProperty is property name as labeleq
	LabelEqualProperty = "labeleq"
)

// Query Options for GenericDataSelect which takes []GenericDataCell and returns selected data.
...
// FilterQuery filter by values
type FilterQuery struct {
	FilterByList []FilterBy
}

// FilterBy specific filter implementation
type FilterBy struct {
	Property PropertyName
	Value    ComparableValue
}

// NoFilter default NoFilter query
...
func GeSimpleLabelQuery(key, value string) *Query {
	return NewDataSelectQuery(
		NoPagination,
		NoSort,
		NewFilterQuery([]string{"label",
			fmt.Sprintf("%s:%s", key, value)}),
	)
}

// GeSimpleFieldQuery get query by 'key == value'
func GeSimpleFieldQuery(key, value string) *Query {
	return NewDataSelectQuery(
		NoPagination,
		NoSort,
...
// ToListOptions is a function which converting query to metav1 listoptions
func (dsQuery *Query) ToListOptions() (ls metav1.ListOptions) {
